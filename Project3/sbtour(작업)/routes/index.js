...
passport.use(new LocalStrategy({
    ...
    pool.getConnection(function(err, connection) {
        ...
        } else if (req.body.user_type == 'seller') { //판매자
            var sqlForLogin = "SELECT * FROM seller WHERE seller_id = ?";
        } else { //관리자
            var sqlForLogin = "SELECT * FROM admin WHERE admin_id = ?";
        }
            ...
            if (err) {
                ...
                if (result.length === 0) {
                    ...
                    if (req.body.user_type == 'customer') { //소비자 
                        ...
                        if (user_passwd != result[0].admin_passwd) {
                            console.log('패스워드가 일치하지 않습니다', req.flash('login_msg', '패스워드가 일치하지 않습니다'));
                            return done(false, null);
                        } else {
                            console.log('로그인 성공');
...
router.post('/product/:package_id', function(req, res, next) {
    ...
    var data2 = [reserve_id, customer_id, package_id, pay_id, reserve_date, reserve_start, reserve_arrive, reserve_people, reserve_price, reserve_status];
    console.log(data1);
    console.log(data2);
    ...
    pool.getConnection(function(err, connection) {
        ...
        var sqlForInsertBoard2 = "INSERT into reservation(reserve_id, customer_id,package_id,pay_id,reserve_date,reserve_start,reserve_arrive,reserve_people,reserve_price,reserve_status) values(?,?,?,?,?,?,?,?,?,?)";
...
router.get('/profile_get', function(req, res, next) {
    ...
    if (type === "seller") {
        ...
        sqlForSelectList1 = "select * from admin where admin_id = ?;"
        render_ = "profile_admin";
    }
...
/*관리자 MY -> 고객 리스트 */
router.get('/admin_customer_list', function(req, res, next) {
    var user_id = req.user.user_id;
    ...
    pool.getConnection(function(err, connection) {
        if (err) return res.sendStatus(400);
        var sqlForSelectList = "select * from customer;";
...
/*관리자 MY -> 예약 관리 */
router.get('/admin_reservation_list', function(req, res, next) {
    var user_id = req.user.user_id;
    ...
    pool.getConnection(function(err, connection) {
        ...
        var sqlForSelectList =
            ...
            "select * from package where package_id in (select package_id from reservation);";
        connection.query(sqlForSelectList, [user_id, user_id], function(err, rows) {
            if (err) console.error("err1 : " + err);
            console.log("rows[0] : " + JSON.stringify(rows[0]));
            console.log("rows[1] : " + JSON.stringify(rows[1]));
            res.render('admin_reservation_list', {
                title: 'admin_reservation_list',
                reservation: rows[0],
                package: rows[1],
                login: login,
                user_id: user_id
...
/*관리자 MY -> 예약 관리 */
router.post('/admin_reservation_list', function(req, res, next) {
    var user_id = req.user.user_id;
    ...
    pool.getConnection(function(err, connection) {
        var sqlForInsertBoard = "delete from reservation where reserve_id = ?";

        connection.query(sqlForInsertBoard, reserve_id, function(err, rows) {
            if (err) console.error("err: " + err);
            res.send('<script type="text/javascript">alert("삭제 완료되었습니다.");history.back();</script>');
            connection.release();
        });
    });
});
...
/*관리자 MY -> 판매자 리스트 */
router.get('/admin_seller_list', function(req, res, next) {
    var user_id = req.user.user_id;
    ...
    pool.getConnection(function(err, connection) {
        if (err) return res.sendStatus(400);
        var sqlForSelectList = "select * from seller;";
        connection.query(sqlForSelectList, function(err, rows) {
            if (err) console.error("err1 : " + err);
            console.log("rows : " + JSON.stringify(rows));
            res.render('admin_seller_list', {
                title: 'admin_seller_list',
                seller: rows,
                login: login,
                user_id: user_id
            });
...
/*관리자 MY -> 판매 상품 관리 */
router.get('/admin_package_list', function(req, res, next) {
    var user_id = req.user.user_id;
    ...
    pool.getConnection(function(err, connection) {
        if (err) return res.sendStatus(400);
        var sqlForSelectList = "select * from package;";
        connection.query(sqlForSelectList, function(err, rows) {
            if (err) console.error("err1 : " + err);
            console.log("rows : " + JSON.stringify(rows));
            res.render('admin_package_list', {
                title: 'admin_package_list',
                package: rows,
                login: login,
                user_id: user_id
            });
...
/*관리자 MY -> 판매 상품 관리 */
router.post('/admin_package_list', function(req, res, next) {
    var user_id = req.user.user_id;
    var package_id = req.body.package_id;
    ...
    pool.getConnection(function(err, connection) {
        var sqlForDeleteReservation = "delete from reservation where package_id = ?;"
        var sqlForDeletePackage = "delete from package where package_id = ?";

        pool.getConnection(function(err, connection) {

            connection.query(sqlForDeleteReservation, package_id, function(err, tour_id) { //예약 삭제
                if (err) console.error("err: " + err);
                connection.query(sqlForDeletePackage, package_id, function(err, rows) { //패키지 삭제
                    if (err) console.error("err: " + err);
                    res.send('<script type="text/javascript">alert("삭제 완료되었습니다.");history.back();</script>');
                });
                connection.release();
